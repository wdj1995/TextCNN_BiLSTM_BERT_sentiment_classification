...
def train(train_iter, dev_iter, model, args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for i, batch in enumerate(train_iter):
            ...
            # logit = model(feature, feature_lengths.to("cpu")) ############### LSTM 特殊结构决定
            logit = model(feature).to(device) ############### LSTM 特殊结构决定
            ...
            # loss = criterion(logit.squeeze(-1), target.float()) # 二分类损失函数
            ...
            loss.backward()
            torch.backends.cudnn.enabled = False  ####################防止报错
            ...
            steps += 1
        print("{} epoch of steps is :".format(epoch), steps+1)
        ...
        ############################################### 开始validation ###############################################
        val_total_acc, val_avg_loss, val_total_f1score, val_total_precision, val_total_recall, val_total_bcm = test(dev_iter, model)
