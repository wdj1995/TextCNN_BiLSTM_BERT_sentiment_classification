...
def elbow_method(time_series_data, max_cluster, seed):
    ...
    for i in range(1, max_clusters + 1):
        kmeans = KMedoids(n_clusters=i, random_state=seed, metric="euclidean") # metric="precomputed"
        kmeans.fit(X_reshaped)
        SSE.append(kmeans.inertia_)
    # 绘制手肘图
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, max_clusters + 1), SSE, marker='o', linestyle='--')
    print("KMedoids+euclidean+Elbow Method：", SSE)
    plt.title('Elbow Method For Optimal k')
    plt.xlabel('Number of clusters (k)')
    plt.ylabel('SSE')
    plt.xticks(range(1, max_clusters + 1))
    plt.grid(True)
    plt.savefig('KMedoids+euclidean+Elbow Method.jpeg', dpi=600)
    plt.show()
def silhouette_coefficient(time_series_data, max_clusters, seed):
    # 初始化TimeSeriesScalerMeanVariance
    scaler = TimeSeriesScalerMeanVariance()
    # 对时间序列数据进行标准化处理
    X = scaler.fit_transform(time_series_data)
    ...
    for n_clusters in range(2, max_clusters + 1):
        km = KMedoids(n_clusters=n_clusters, metric="euclidean", random_state=seed) # # metric="precomputed"
    ...
    print("KMedoids+euclidean:", silhouette_scores)
    plt.title('Silhouette Scores For Optimal k')
    plt.xlabel('Number of clusters (k)')
    plt.ylabel('Silhouette Score')
    plt.grid(True)
    plt.savefig('KMedoids+euclidean+silhouette_scores.jpeg', dpi=600)
    plt.show()
def kmedoids(time_series_data, k, seed):
    # 初始化TimeSeriesScalerMeanVariance
    scaler = TimeSeriesScalerMeanVariance()
    # 对时间序列数据进行标准化处理
    X = scaler.fit_transform(time_series_data)
    ...
    cluster_centers = km.medoid_indices_
    ...
    for cluster in range(n_clusters):
        ...
        for series in X_reshaped[labels == cluster]:
            plt.plot(series, color='gray', alpha=0.5)
        plt.plot(X_reshaped[cluster_centers[cluster]], color='#D10363', lw=2)
...
def elbow_method_dtw(time_series_data, max_cluster, seed):
    ...
    max_clusters = max_cluster
    X_reshaped = X.reshape((X.shape[0], -1))
    dists = metrics.cdist_dtw(X_reshaped)  # dba + dtw

    for i in range(1, max_clusters + 1):
        km = KMedoids(n_clusters=i, random_state=seed, metric="precomputed") # metric="precomputed"
        km.fit(dists)
        SSE.append(km.inertia_)
    # 绘制手肘图
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, max_clusters + 1), SSE, marker='o', linestyle='--')
    print("KMedoids+dtw+Elbow Method", SSE)
    plt.title('Elbow Method For Optimal k')
    plt.xlabel('Number of clusters (k)')
    plt.ylabel('SSE')
    plt.xticks(range(1, max_clusters + 1))
    plt.grid(True)
    plt.savefig('KMedoids+dtw+Elbow Method.jpeg', dpi=600)
    plt.show()
def silhouette_coefficient_dtw(time_series_data, max_clusters, seed):
    import tslearn.metrics as metrics
    # 初始化TimeSeriesScalerMeanVariance
    scaler = TimeSeriesScalerMeanVariance()
    # 对时间序列数据进行标准化处理
    ...
    for n_clusters in range(2, max_clusters + 1):
        km = KMedoids(n_clusters=n_clusters, metric="precomputed", random_state=seed) # # metric="precomputed"
        labels = km.fit_predict(dists)
        silhouette_avg = silhouette_score(X, labels, metric="dtw")
        silhouette_scores.append(silhouette_avg)
    # 绘制轮廓系数图
    plt.figure(figsize=(10, 6))
    plt.plot(range(2, max_clusters + 1), silhouette_scores, marker='o')
    print("KMedoids+dtw+silhouette_scores", silhouette_scores)
    plt.title('Silhouette Scores For Optimal k')
    plt.xlabel('Number of clusters (k)')
    plt.ylabel('Silhouette Score')
    plt.grid(True)
    plt.savefig('KMedoids+dtw+silhouette_scores.jpeg', dpi=600)
    plt.show()
def kmedoids_dtw(time_series_data, k, seed):
    import tslearn.metrics as metrics
    # 初始化TimeSeriesScalerMeanVariance
    scaler = TimeSeriesScalerMeanVariance()
    # 对时间序列数据进行标准化处理
...
if __name__ == '__main__':
    ...
    ################################# K-Medoids clustering #############################################
    # 使用手肘法判断聚类个数
    elbow_method(result_time_arr,12,123)
    # 使用轮廓系数
    silhouette_coefficient(result_time_arr,12, 123)
    # 通过手肘法和轮廓系数法判断，聚类个数为3 最佳
    # kmedoids(result_time_arr,2, 123)

    # # 使用手肘法判断聚类个数
    elbow_method_dtw(result_time_arr,12,123)
    # # 使用轮廓系数
    silhouette_coefficient_dtw(result_time_arr,12,123)
    # # # 通过手肘法和轮廓系数法判断，聚类个数为3 最佳
    # # # kmedoids_dtw(result_time_arr,2, 123)
    ################################# K-Medoids clustering #############################################
